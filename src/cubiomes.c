...
void cubiomes_free(Cubiomes *cubiomes) { free(cubiomes); }

void cubiomes_apply_climate(Cubiomes *cubiomes, uint64_t seed,
                            uint32_t octaves) {
  setClimateParaSeed(&cubiomes->g.bn, seed, 0, NP_CONTINENTALNESS, octaves);
}

int cubiomes_is_surrounded(Cubiomes *cubiomes, int startx, int startz, int step,
                           double threshhold) {
...
int cubiomes_locate_climate_extreme(Cubiomes *cubiomes, int32_t x, int32_t z,
                                    int32_t range) {
  double pmax;
...
int cubiomes_test_monte_carlo(Cubiomes *cubiomes, int32_t x, int32_t z,
  ...
  return monteCarloBiomes(&cubiomes->g, r, &rng, fraction, confidence, eval,
                          NULL);
}

int cubiomes_test_biome_centers(Cubiomes *cubiomes, int32_t x, int32_t z,
                                int32_t range, int32_t min_area, PosArea *out) {
  Pos pos;
  int siz;
  Range r = make_range_s4(x, z, range);
  int n = getBiomeCenters(&pos, &siz, 1, &cubiomes->g, r, mushroom_fields,
                          min_area / 16, 1, NULL);
  if (n == 1) {
    out->x = pos.x;
    out->z = pos.z;
